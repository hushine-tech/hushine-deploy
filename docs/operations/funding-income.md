...
- 每个 poller 进程只维护内存 watermark；首次运行或进程重启时没有持久化 watermark，
  查询从 `now - 24h overlap` 恢复；成功轮询后才把内存 watermark 推进到本轮 `now`，
  后续仍从该值前 24 小时重叠读取并分页到结尾；
...
运维面应检查：各状态行数、最老 pending age、24 小时 grace 超时、非零
`reconciliation_delta`、poll error、Income delivery cursor lag、Worker ACK/backpressure 和 Spot
Funding 行数（必须为零）。当前实现只把 poller sweep/terminal error、24 小时 grace 到期和
Funding Account handler error 写入 core-service 结构化错误日志；retryable poll error 只按
退避重试。`reconciliation_delta` 会被持久化，但尚无非零 delta 检测告警；外部或手工账户
变化也尚无专门检测/告警。尚无独立 Funding metrics exporter，dashboard 只能从 current
Income 状态和上述现有错误日志派生，不能把“没有专用指标”解释为健康。

日志只保留 route/session/entry 等业务 ID，不记录 secret、签名 query 或完整 credential。
...
blocked；不得用临时 curl、直接数据库修改或 Live credential 代替。Task 16 gate 必须拒绝
Live，并从一次性匿名 inherited read-only FD 读取 credential JSON，读取后立即关闭；不得
要求 API key、secret、Venue ID 或 Session ID 使用四个命名环境变量传递。非秘密的 Venue/
Session 标识使用显式参数，查询时间窗必须受限，输出只允许脱敏摘要。
