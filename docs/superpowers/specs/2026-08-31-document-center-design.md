...
```text
左侧文档目录 | 中间 Markdown 阅读区 | 右侧短会话问答区
```
...
问答只保证当前浏览器标签页内的短时间连续性：

- 前端把最近对话保存在该标签页的 `sessionStorage`，不写业务数据库。
- 同一标签页内切换文档、返回文档首页或刷新页面，30 分钟内可以继续追问。
- 关闭标签页、退出登录、用户点击“清空会话”或连续 30 分钟无操作后清空。
- 最多携带最近 12 轮问答；更早内容从请求上下文中移除。
- 不提供历史会话列表、新建/切换会话、跨设备同步或部署版本间的旧会话回放。
- `quant-handler` 不保存会话对象。每次请求只接收受限长度的最近消息，因此服务重启和多实例不会产生
  会话粘滞问题。
...
阶段二增加服务端 OpenAI Responses API 调用、受权限约束的文档/源码检索、短会话上下文和引用展示。
...
- `messages` 最多 24 条，即 12 轮；单条和总字符数都有硬上限；
...
- Mock OpenAI 覆盖正常回答、工具检索、429、超时、错误引用和超过 12 轮上下文。
...
- 同一标签页切换文档和刷新后短会话连续；关闭、退出、清空或 30 分钟超时后重置。
- 超过 12 轮时只发送最近 12 轮；当前文档切换后正确附带新的 document ID。
...
- 页面内短会话在当前标签页 30 分钟内连续，不产生长期会话数据；
