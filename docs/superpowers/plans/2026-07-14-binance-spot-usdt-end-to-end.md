...
**Architecture:** Core-service remains the only exchange and order authority. Route-aware Binance adapters read `/api/v3/account` and `/api/v3/exchangeInfo`, preserve exact decimal rules, and expose canonical asset balances plus immutable metadata snapshots to preflight, risk, execution, stop-and-close, and reconciliation. Strategy workers and offline replay consume those contracts without credentials or exchange access; control-panel only proxies authenticated RuntimeChannel calls; quant-handler maps identity and wire data without inventing exchange semantics; quant-frontend presents the supported route and structured failures.

**Tech Stack:** Go 1.26.1, Protocol Buffers/gRPC, Python 3.13 with `uv`/pytest, TypeScript/React/Vite, Binance Spot REST Demo/Testnet, TimescaleDB JSON snapshots, Bash, Docker coverage images, and the installed Browser skill plus `browser-client` raw-CDP acceptance handoff.
...
| `core-service` | `internal/order/executor/{executor,adapter_router}.go`, `internal/exchange/binance/{order_executor,order_state_reader,order_capability}.go`, `internal/order/{service,lifecycle,repository}/*` | Exact request construction, fee asset propagation, monotonic/idempotent lifecycle, and persisted fills. |
| `core-service` | new `internal/order/spotclose/{types,planner}.go`, `internal/reconciliation/{diff,service}.go` | Atomic preplan for declared Spot targets and authoritative post-trade reconciliation. |
...
| `strategy-library` | new `hushine_strategy/wallet/spot.py`, `hushine_strategy/wallet/portfolio.py`, modify `wallet/__init__.py`, `replay/engine.py` | Hosted-equivalent offline Spot and mixed-route replay. |
...
| `gateway/quant-handler` | `internal/app/{venues,wallet_bootstrap,portfolios_ext,strategy,debug_package,debug_package_parquet,order_history,session_history,enum_labels}.go` | Identity/authorization mapping, canonical Spot payloads, exact order-history decimals, server-side Live guard, and debug package v2. |
| `gateway/quant-frontend` | `src/api/client.ts`, `src/pages/{VenueManagement,PortfolioDetail,SessionDetailPage}.tsx`, `src/components/{StopSessionDialog,SymbolPicker}.tsx`, new tracked `scripts/spot-*.test.mjs` | Spot asset UI, route-aware start/debug/stop, structured errors, and visible Live guard. |
| `hushine-deploy` | new `scripts/verify_spot_usdt.sh`, `scripts/verify_spot_usdt.test.sh`, `scripts/smoke_spot_demo.sh`, `scripts/smoke_spot_demo.test.sh`, `scripts/lib/runtime_coverage.sh`, `docs/spot-usdt.md`, `docs/acceptance/2026-07-14-binance-spot-demo.md`; modify `scripts/smoke_hosted_runtime_coverage.sh`, `scripts/smoke_hosted_runtime_coverage.test.sh`, generated `db/generated/order.sql`, `db/generated/README.md`, `db/README.md`, and `docs/{runtime-operator-flow,strategy-debugger-cli-smoke,production-deploy-checklist}.md` | Reproducible Backtest/Demo/offline/UI/filter/stop gates, regenerated order schema, shared safe coverage finalization, Browser-skill handoff, operator/user documentation, and rollback instructions. |
...
**Interfaces:**
- Replace float-only `SymbolRule` values with exact typed metadata:
...
- Add `ReadSymbolRulesSnapshot(ctx, req) (SymbolRulesSnapshot, error)` where the snapshot has `ReadAt`, immutable `RulesBySymbol`, endpoint, and environment identity. Preserve `permissionSets` exactly as nested sets and reject empty inner sets or malformed members. Keep the old slice method as a thin compatibility wrapper only until all call sites migrate in Task 4.
...
```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/core-service
go test ./internal/exchange/binance/... ./internal/catalog ./internal/service -count=1
...
### Task 4: Enforce Spot preflight and Live admission with one metadata snapshot
...
- Extend `RequiredSymbol` additively with `bool order_target` and `repeated string required_order_types`; strategy-service fills `MARKET` and `LIMIT` for declared targets because those are the two platform-supported dynamic decisions.
...
Add account permission, status, spotTradingAllowed, required order type, Spot-only key, Backtest no-network, Demo route, and same-symbol Spot/Futures isolation cases. Cover official permission semantics explicitly: `[[SPOT,MARGIN]]` admits an account with either permission; `[[SPOT],[TRD_GRP_004,TRD_GRP_005]]` requires SPOT plus at least one trading-group permission; an account with only one outer set fails. The old symbol `permissions=[]` must neither reject nor admit when valid `permissionSets` are present.

Run:

```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/core-service
go test ./cmd/core-service ./internal/service ./internal/exchange/binance -run 'TestPreflightSpot|TestRegistryRejectsLiveSpot|TestSpotOnlyCredential' -count=1
```
...
Expected GREEN: Backtest performs no network I/O, Demo Spot-only credentials pass, invalid targets produce exact codes in a single response, Live Spot fails before exchange I/O, and Futures remains available.
...
git add cmd/core-service/exchange_registry.go cmd/core-service/exchange_registry_test.go internal/service/grpc.go internal/service/grpc_portfolio_meta_test.go internal/service/portfolio_snapshot_test.go internal/exchange/binance/factory_test.go
...
- Add `OrderErrorDetail error = 8` to `PlaceOrderResponse` and `ResolveOrderAttemptResponse`, preserving the legacy `error_message`. `OrderErrorDetail` fields are `code = 1`, `message = 2`, `venue_id = 3`, `exchange = 4`, `market = 5`, `symbol = 6`, and `filter_type = 7`.
...
Require the risk gate to load the same route's Spot rules before checking balances. For percent-price or market notional filters, fetch the trusted average price inside core-service and reject a missing/mismatched `mins` response; do not trust a worker-supplied mark as the exchange reference. Build Binance `quantity` and `price` URL parameters directly from validated strings; never round-trip through `float64`. Persist the same authoritative strings by passing them through intent, attempt, risk audit, and recovery models; SQL casts/binds them to `numeric(38,18)`, and reads use `numeric_column::text` (or an equivalent lossless numeric scanner), never `float64`.
...
```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/core-service
go test ./internal/order/risk ./internal/order/executor ./internal/exchange/binance -count=1
...
- Reconciliation compares each canonical Spot asset's exact free and locked balances for the same venue route. Missing account data or permission errors are hard failures; valuation-price absence is advisory only.
...
- Because `order_service.proto` now imports `portfolio_service.proto`, `strategy-service/generate_proto.sh` must rewrite the generated Python dependency to `from . import portfolio_service_pb2 as portfolio__service__pb2` (and keep the existing relative imports) on both GNU and BSD/macOS sed paths. A generated-import test imports `strategy_service.gen.order_service_pb2` from outside the repository cwd.
...
Add SELL, base commission, quote commission, partial-to-filled, cancel/expire release, late NEW, insufficient USDT, metadata absence fail-closed, legacy snapshot import, same-symbol Spot/Futures, multiple symbols, and two intervals of one symbol.
...
PYTHONPATH=.:../strategy-library uv run --frozen --extra dev pytest tests/test_wallet_runtime.py tests/test_wallet_strict_rules.py tests/test_portfolio_wallet_runtime.py tests/test_order_client.py tests/test_platform_proxy.py tests/test_strategy_engine.py tests/test_grpc_server.py tests/test_preflight.py tests/test_spot_end_to_end.py tests/test_generated_proto_imports.py -q
```
...
First extend the portable relative-import rewrite in `generate_proto.sh` for `order_service_pb2.py`'s new portfolio import and add an isolated import test; do not hand-edit generated code. Use `Decimal` created from protocol strings. Keep a per-order cumulative state record under the full route key. For BUY subtract cumulative quote delta, add cumulative base delta, then subtract commission from its actual asset; for SELL subtract cumulative base delta, add cumulative quote delta, then subtract commission. Reject an event whose metadata route does not equal the order route.
...
Expected GREEN: canonical Backtest and Demo scenarios pass, actual fee assets apply once, two intervals do not duplicate wallet changes, and Spot/Futures same-symbol orders stay isolated.

- [ ] **Step 4: Commit strategy-service only**

```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/strategy-service
git add generate_proto.sh strategy_service/gen/portfolio_service_pb2.py strategy_service/gen/portfolio_service_pb2_grpc.py strategy_service/gen/order_service_pb2.py strategy_service/gen/order_service_pb2_grpc.py gen/portfoliov1/portfolio_service.pb.go gen/portfoliov1/portfolio_service_grpc.pb.go strategy_service/wallet/canonical.py strategy_service/wallet/spot.py strategy_service/wallet/binance.py strategy_service/wallet/portfolio.py strategy_service/wallet/portfolio_adapter.py strategy_service/wallet/order_types.py strategy_service/order_client.py strategy_service/platform_proxy.py strategy_service/grpc_server.py internal/runtimeagent/session_lifecycle.go internal/runtimeagent/session_lifecycle_test.go internal/runtimeagent/agent_test.go tests/test_wallet_runtime.py tests/test_wallet_strict_rules.py tests/test_portfolio_wallet_runtime.py tests/test_order_client.py tests/test_platform_proxy.py tests/test_strategy_engine.py tests/test_grpc_server.py tests/test_preflight.py tests/test_spot_end_to_end.py tests/test_generated_proto_imports.py
...
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/core-service
test -n "$HUSHINE_TEST_PG_ADMIN_DSN"
HUSHINE_TEST_PG_ADMIN_DSN="$HUSHINE_TEST_PG_ADMIN_DSN" go test -tags=integration ./internal/order/storage/migrations ./internal/order/spotclose -run 'TestSpotCloseOperationsMigration|TestSpotCloseOperationPersistence' -count=1 -v
...
```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/core-service
go test ./internal/order/... ./internal/exchange/binance -count=1
...
- The Python worker never persists `stopped`, `stop_failed`, or `recoverable`. It returns the stop operation result and emits one typed desired `FinalStatus` carrying status/reason/reconciliation identity. The Indicator V2 `SessionLifecycle` in the Go Agent remains the sole external terminal publisher: it closes frame admission, finalizes the indicator tail, then persists the desired stop state. A successful exchange close cannot bypass or race indicator finalization.
...
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/strategy-service
PYTHONPATH=.:../strategy-library uv run --frozen --extra dev pytest tests/test_order_client.py tests/test_platform_proxy.py tests/test_grpc_server.py -q -k 'spot and stop'
```

Expected RED: the proxy method is unknown and current worker stop rejects Spot with `spot_liquidation_not_supported`.

- [ ] **Step 2: Implement authenticated pass-through and worker state handling**

Reuse the same canonical protobuf JSON envelope and timeout/error mapping as `order.PlaceOrder`. Delete the worker's symbol-suffix liquidation builder for Spot. Do not emit desired `stopped` until core confirms all planned Spot targets and existing Futures close work succeeded; emit desired `stop_failed` plus reconciliation identity otherwise. Route both through the existing Agent terminal coordinator and keep Python's agent-managed terminal-persistence guard enabled for every stop outcome.
...
Add multi-symbol, same-symbol two-interval, same route facts with distinct `stream_id`, same symbol with distinct `kind`, input-only no-order, undeclared route, metadata absence, USDT quote restriction, partial/cancel lifecycle, and Futures regression cases.

Run:

```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/strategy-library
uv run --isolated --no-project --with-editable '.[test]' pytest tests/hushine_strategy/test_spot_wallet.py tests/hushine_strategy/test_mixed_route_replay.py tests/hushine_strategy/test_replay.py -q
```
...
Do not import `strategy-service` from the library. Keep the contract parity explicit with shared scenario fixtures and values, and add a contract test in `strategy-service/tests/test_strategy_engine.py` that runs the same fill vector through both engines and compares canonical JSON.
...
```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/strategy-library
git add hushine_strategy/wallet/spot.py hushine_strategy/wallet/portfolio.py hushine_strategy/wallet/__init__.py hushine_strategy/replay/engine.py hushine_strategy/replay/__init__.py tests/hushine_strategy/test_replay.py tests/hushine_strategy/test_spot_wallet.py tests/hushine_strategy/test_mixed_route_replay.py
...
Add tampered hash, absent Spot metadata, missing interval file, duplicate stream ID, duplicate full route, same route facts with distinct stream IDs, same symbol/interval with distinct kinds, undeclared data file, v1 Futures compatibility, same-symbol mixed-market, multi-symbol/multi-interval, and package reproducibility cases.
...
./scripts/with-local-strategy-library-git.sh ../strategy-library \
  uv run --frozen --extra test pytest tests/test_import_package.py tests/test_replay_cli.py tests/test_spot_package_v2.py tests/test_mixed_route_package_v2.py -q
```
...
Add canonical bootstrap validation, legacy read compatibility, Spot Demo pass-through, mixed route payload, same-symbol route isolation, structured preflight error propagation, stop-only, stop-and-close, metadata-based valuation, and order/session-history cases that preserve `9007199254740993.00000000`, `0.00000001`, cumulative quote, and BNB fee strings without rounding.

Run:

```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/gateway/quant-handler
go test ./internal/app ./internal/walletagg -run 'Test.*Spot|TestStartSessionRejectsLiveSpot' -count=1
```
...
Keep exact strings in request DTOs until protobuf mapping. Normalize asset codes to uppercase only after validating Binance asset syntax; do not convert an asset into a trading symbol. Map core structured error code, route, symbol, filter type, and message unchanged to the HTTP error body.

- [ ] **Step 3: Make HTTP contract tests GREEN**

```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/gateway/quant-handler
gofmt -w internal/app/venues.go internal/app/venues_test.go internal/app/wallet_bootstrap.go internal/app/portfolios_ext.go internal/app/portfolios_ext_test.go internal/app/strategy.go internal/app/strategy_test.go internal/app/strategy_cutover_test.go internal/app/order_history.go internal/app/order_history_test.go internal/app/session_history.go internal/app/session_history_test.go internal/app/enum_labels.go internal/walletagg
go test ./internal/app ./internal/walletagg -run 'Test.*Spot|TestStartSessionRejectsLiveSpot' -count=1
```

Expected GREEN: HTTP payloads contain asset codes only, exact values survive, Spot Demo is preserved, Live Spot fails before proxying, and structured core errors reach the caller.
...
- Session detail groups wallet/order/fill/error rows by venue/exchange/market and shows filter/error code plus fee asset. Order and fill cells render the exact decimal strings from the API; TypeScript types retain them as strings and never coerce them with `Number`, `parseFloat`, or arithmetic.
...
Expected GREEN: all five contracts pass, TypeScript compiles, and Vite produces a production build without route/type casts that erase Spot.
...
**Interfaces:**
- `verify_spot_usdt.sh` accepts exactly one scope: `backtest`, `demo`, `offline`, `ui`, `filters`, `stop`, or `all`. It resolves the verified source root and dispatches to the exact focused tests named by this plan. `demo` additionally takes one absolute coverage-output directory. Every pre-push debugger invocation in `offline`/`all` runs through `strategy-debugger-cli/scripts/with-local-strategy-library-git.sh` against that root's strategy-library, asserts the lock pin equals final library HEAD, and never records the mirror transport.
- `smoke_spot_demo.sh` requires `USER_ID`, `PORTFOLIO_ID`, an absolute coverage-output directory, and one already-created run-owned Demo `VENUE_ID`; it rejects every Binance key/secret environment variable. The full-system acceptance provisioning helper—not this smoke—owns authenticated encrypted Venue creation and ambiguous-response recovery.
- The smoke proves core reached `/api/v3/account`, `/api/v3/exchangeInfo`, `/api/v3/myFilters`, and the reference-price-then-average-price fallback path through Venue-backed product calls, starts an instrumented Hosted Runtime, runs a deterministic two-symbol/two-interval strategy, submits metadata-derived rule-valid MARKET BUY and SELL orders, observes fills/fee assets/wallet/reconciliation, tests stop-only and stop-and-close in separate Sessions, exports/imports/replays package v2 with networking disabled, and cleans up only resources carrying its run ID.
...
`smoke_spot_demo.test.sh` must assert strict mode, absence of `set -x`, mandatory
...
reconciliation, coverage finalization, run-ID ownership checks, and cleanup.

`verify_spot_usdt.test.sh` must assert the exact seven-scope allow-list and command mapping, require the local-library wrapper plus final pin check for every debugger subprocess, and reject an unknown scope before starting a subprocess.
...
Expected RED: the two new scripts do not exist, and each contract test reports the first missing required behavior.
...
mkdir -p hushine-deploy/scripts/lib hushine-deploy/docs/acceptance
```
...
|---|---|
| `backtest` | `strategy-service/tests/test_spot_end_to_end.py` plus `strategy-library/tests/hushine_strategy/test_mixed_route_replay.py` |
| `demo` | `hushine-deploy/scripts/smoke_spot_demo.sh` |
| `offline` | `strategy-debugger-cli/tests/test_spot_package_v2.py` and `test_mixed_route_package_v2.py` |
| `ui` | every `gateway/quant-frontend/scripts/spot-*.test.mjs` followed by `npm run build` |
| `filters` | `core-service/internal/order/risk/decimal_test.go` and `spot_filters_test.go` |
...
| `all` | all non-Demo scopes in the table, then Demo only when its required absolute output path and Venue/one-time credentials are present |
...
Run `make db-schema-bundle` after Tasks 6 and 8. The generated `db/generated/order.sql` and source inventory must contain `0000`, the byte-identical deployed `0001_current_schema_baseline.sql`, `0002_spot_order_route_identity.sql`, and `0003_spot_close_operations.sql` exactly once and in order. Record and compare the source `0001` SHA-256 to the pre-task baseline; any change is a hard failure. A second generation must produce identical checksums. Update `db/README.md` with the additive order migration order, pre/post row-count and identity checks, rollback-by-disabling-Spot rule, and the prohibition on deleting order/fill/close-operation history.
...
HUSHINE_TEST_PG_ADMIN_DSN="$HUSHINE_TEST_PG_ADMIN_DSN" \
  go test -tags=integration ./internal/order/storage/migrations ./internal/order/spotclose ./internal/order/repository ./cmd/ensure-order-db \
  -run 'TestSpotOrderRouteIdentityMigration|TestSpotCloseOperationsMigration|TestSpotCloseOperationPersistence|TestOrderRepositoryExactSpotRoundTrip|TestEnsureOrderMigrationAtomic' -count=1 -v
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/strategy-service
...
bundle_before="$(shasum -a 256 db/generated/order.sql db/generated/README.md)"
make db-schema-bundle
test "$bundle_before" = "$(shasum -a 256 db/generated/order.sql db/generated/README.md)"
```
...
```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup
install -d -m 0700 census-runs/spot-demo-20260714/coverage/runtime-agent
USER_ID="$USER_ID" \
PORTFOLIO_ID="$PORTFOLIO_ID" \
VENUE_ID="$VENUE_ID" \
COVERAGE_IMAGE=hushine/strategy-runtime:executor-coverage-spot-acceptance \
./hushine-deploy/scripts/verify_spot_usdt.sh demo \
  /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/census-runs/spot-demo-20260714/coverage/runtime-agent
```
...
```bash
cd /Users/xdy/Workplace/hushine-worktrees/medium-cleanup/hushine-deploy
git add scripts/verify_spot_usdt.sh scripts/verify_spot_usdt.test.sh scripts/smoke_spot_demo.sh scripts/smoke_spot_demo.test.sh scripts/lib/runtime_coverage.sh scripts/smoke_hosted_runtime_coverage.sh scripts/smoke_hosted_runtime_coverage.test.sh db/generated/order.sql db/generated/README.md db/README.md docs/spot-usdt.md docs/acceptance/2026-07-14-binance-spot-demo.md docs/runtime-operator-flow.md docs/strategy-debugger-cli-smoke.md docs/production-deploy-checklist.md
...
This focused implementation plan is complete only when all of the following are true:

- Official `/api/v3/account` and `/api/v3/exchangeInfo` contract tests pass; no production or mock Spot flow uses `/api/v3/portfolio` or `/fapi/*`.
- Canonical wallets contain asset codes only and retain exact free/locked values; `BTCUSDT` never appears as an asset.
- Exact request/order/fill decimals survive protocol, numeric persistence, handler JSON, and UI rendering; conflict cases fail before side effects.
...
- Exact filter checks, BUY/SELL fills, fee assets, idempotent lifecycle, reconciliation, and approved stop matrices pass; accepted-timeout/restart/lease-expiry races issue at most one SELL and block unrelated orders until authoritative recovery.
- Hosted Backtest, offline debugger, UI contracts, multi-symbol, multi-interval, and mixed Spot/Futures focused gates pass.
- Live Spot remains fail-closed at core, handler, and UI boundaries.
- The coverage image, seven-scope verifier, Venue-backed real Demo smoke, tracked handoff/runbook, rolling compatibility tests, and rollback instructions are ready for the full-system acceptance plan.
...
- Real Demo execution, Browser-skill same-tab raw-CDP evidence, shared-infrastructure/fresh-bootstrap validation, Notion synchronization, remote pushes, and final rollout remain explicit full-system acceptance gates; this plan does not claim them complete.
