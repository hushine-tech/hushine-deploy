...
FUT-WALLET-FULL	Futures full lifecycle, persisted duplicate replay, and bounded checkpoint retention	Exact fee/wallet and position delta once across canonical/proto/authoritative refresh; no open order; at most 1024 checkpoints	pytest:tests/test_trading_mode_wallet_matrix.py::test_futures_canonical_order_outcome_wallet_delta_matrix[full];pytest:tests/test_trading_mode_wallet_matrix.py::test_futures_terminal_replay_remains_idempotent_after_canonical_round_trip;pytest:tests/test_trading_mode_wallet_matrix.py::test_futures_order_checkpoint_retention_is_bounded;pytest:tests/test_wallet_runtime.py::test_terminal_order_checkpoint_survives_proto_serialize_and_rebuild;pytest:tests/test_portfolio_snapshot_adapter.py::test_authoritative_futures_snapshot_preserves_local_terminal_order_checkpoint
...
actual_for_cell() {
  case "$1" in
    ...
    FUT-WALLET-FULL)
      echo "wallet=999.96; position qty=1; open=0; runtime/canonical/proto/authoritative replay delta=0; retained checkpoints=1024 max" ;;
